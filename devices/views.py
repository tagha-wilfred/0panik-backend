...
class DeviceIngestView(APIView):
    """
    Endpoint for ESP32 devices to submit location pings.
    Uses custom device authentication and rate limiting.
    """
    authentication_classes = [DeviceAPIKeyAuthentication]
    permission_classes = []  # no user permission required
    throttle_classes = [DeviceIngestRateThrottle]  # ← Use custom throttle
    throttle_scope = 'device_ingest'
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {'detail': 'latitude must be -90..90, longitude -180..180'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if recorded_at:
            try:
                ...
                return Response(
                    {'detail': 'recorded_at must be ISO 8601 datetime'}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
        else:
            recorded_at = timezone.now()
