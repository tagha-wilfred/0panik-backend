...
class URLSecurityAnalyzer:
    ...
    SHORTENERS = [
        ...
        'lnkd.in', 'g.co', 'm.me', 'fb.com', 'tinyurl', 'shorturl'
    ]
    
    # Suspicious TLDs
    SUSPICIOUS_TLDS = ['.tk', '.ml', '.ga', '.cf', '.top', '.xyz', '.club', '.online', '.site', '.space', '.click', '.download']
    ...
    def _check_domain_age(cls, domain: str) -> dict:
        ...
        try:
            ...
            elif w.creation_date is None:
                # Domain might be private or WHOIS lookup failed
                return {
...
class ChatbotCheckView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        # --- Stage 1: Safe Browsing Check (External API) ---
        ...
        if urls:
            ...
            if safe_browsing_result:
                ...
                overall_reason.append(safe_browsing_result.get('message', 'Flagged by Safe Browsing'))
                source = 'safe_browsing'
                safe_browsing_data = {'flagged': True}
        ...
        if not safe_browsing_flagged and urls:
            ...
            if severity in ['critical', 'high']:
                overall_verdict = 'risky'
            elif severity in ['medium_high', 'medium'] and overall_verdict == 'safe':
            ...
            for finding in analysis.get('findings', []):
                ...
                elif finding.get('severity') in ['medium_high', 'medium'] and not overall_reason:
                    overall_reason.append(finding.get('message', ''))
        ...
        # Truncate reason to avoid massive responses
        if overall_reason:
            reason_text = ' | '.join(overall_reason[:3])
        else:
            reason_text = 'No specific issues detected'
        ...
        try:
            ...
            # Create a mock object for response
            class MockScamCheck:
        ...
        return Response({
            ...
            'reason': reason_text,
            'source': source,
            'checked_at': scam_check.created_at.isoformat() if hasattr(scam_check, 'created_at') else timezone.now().isoformat(),
            'url_checked': url_checked,
            'submitted_text': text,
            'findings': all_findings[:5],  # Return top 5 findings for debugging
    ...
    def _check_safe_browsing(self, url: str) -> dict:
        ...
        try:
            ...
            if not getattr(settings, 'GOOGLE_SAFE_BROWSING_API_KEY', ''):
        ...
        for keyword, reason in phishing_keywords:
            if keyword in text_lower:
                return {
                    ...
                    'detail': f'Text contains "{keyword}" which is commonly used in scams - {reason}'
                }
        return None
